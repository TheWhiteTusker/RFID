...
const ALIASES = {
  "game-box": "gamebox",
  "infinity-square": "infinity-rectangle",
  "slim-tictactoe": "tea-coaster",
};
...
export async function get(slug) {
  const actual = resolveSlug(slug);
  return (await load())[actual] ?? null;
}
...
export async function neighbour(slug, step) {
  ...
  const i = list.indexOf(resolveSlug(slug));
